...
import type { Presentation, TenantConfig } from '../../store/config/configState';
...
export function resolveSlideDurationMs(
  ...
  const raw = presentation.durationSeconds;
  if (typeof raw !== 'number' || !Number.isFinite(raw) || raw <= 0) {
    return defaultSeconds * 1000;
  }
  const clamped = Math.max(MIN_SLIDE_SECONDS, Math.min(MAX_SLIDE_SECONDS, Math.round(raw)));
  return clamped * 1000;
}
...
export function useDisplayRotation(config: TenantConfig | null): DisplayView {
  ...
  useEffect(() => {
    ...
    function viewForStep(step: number): DisplayView {
      ...
      if (step === 0 || presentations.length === 0) return { kind: 'dashboard' };
      const presentation = presentations[step - 1];
      // Step is beyond the (possibly shrunken) list → fall back to the dashboard.
    ...
    function durationMsForStep(step: number): number {
      ...
      const currentPresentation = configRef.current?.activePresentations?.[step - 1];
    ...
    /** Total steps against the current list: dashboard + one per presentation. */
    function totalSteps(): number {
      return (configRef.current?.activePresentations?.length ?? 0) + 1;
    }
