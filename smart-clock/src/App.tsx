...
} from "./app/components/store/settings/settingsSelectors"
import {
  getDebugViewOverride,
  getDebugRotationFrozen,
} from "./app/components/store/debug/debugSelectors"
...
import { hasAnyWeeklyEvents } from "./app/components/store/config/hasAnyWeeklyEvents"
 ...
 * 3. On ready → render the route's dashboard with the current rotation view.
...
function AppRoot() {
  ...
  // Schedule is an automatic rotation step on tablet only — the TV route shows the
  // calendar permanently, so rotating to it there would be a no-op. The debug
  // override can still force the schedule view on TV (handled in the render below).
  const includeSchedule = route !== 'tv' && !scheduleBlocked && hasAnyWeeklyEvents(data);
  const rotationView = useDisplayRotation(effectiveConfig, {
    includeSchedule,
    paused: debugRotationFrozen,
  });
  ...
  if (status === 'ready') {
    ...
    if (view.kind === 'schedule') {
      ...
      const scheduleNode = <ScheduleCalendar daysAhead={daysAhead} title="לוח זמנים" />;

      // On TV the schedule replaces the calendar column so the clock stays visible,
      // mirroring how the messages view behaves.
      return route === 'tv' ? (
        <TvClockView calendarOverride={scheduleNode} />
      ) : (
        <ClockView
          bodyOverride={
            <section className="schedule-body-section">{scheduleNode}</section>
          }
        />
      );
    }
...
/** Application tree. Kept free of side effects so HMR can refresh it safely. */
export function App() {
  return (
    <>
      <AppRoot />
